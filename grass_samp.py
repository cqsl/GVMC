...
class MetropolisSampler_Wedge(MetropolisSampler):
    ...
    def _sample_chain(sampler, model, variables, state, chain_length):
        ...
        def loop_body(sweep_ind, carry):
            ...
            σp = jax.vmap(jax.lax.dynamic_update_slice)(
                σ, σp_I, L * jnp.expand_dims(I, -1)
            )
            prop_row_log_norm = jax.vmap(jax.lax.dynamic_update_slice)(
                row_log_norm,
                jnp.expand_dims(prop_row_log_norm_I, -1),
                jnp.expand_dims(I, -1),
            )

            # log_prob_ratio = 2. * ( jnp.log(jnp.real(det_ratio)) + prop_row_log_norm_I - row_log_norm.at[ jnp.arange(M) , I ].get() )
            log_prob_ratio = 2.0 * (
                jnp.real(jnp.log(det_ratio))
                + jnp.sum(prop_row_log_norm, axis=-1)
                - jnp.sum(row_log_norm, axis=-1)
            )
            ...
            row_log_norm = jnp.where(
                do_accept.reshape(M, 1), prop_row_log_norm, row_log_norm
            )
            accepted += do_accept
