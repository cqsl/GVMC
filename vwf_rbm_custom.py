...
class RBMModPhase_Trans_Inv(nn.Module):
    ...
    def setup(self):
        ...
        self.gamma0 = (1.0 / self.out_std_init) * (
            (2.0 * self.psi_std_init**2) / (self.n_sites * self.features)
        ) ** (0.25)
    ...
    def __call__(self, inputs):
        ...
        x = x.transpose(1, 2, 0)
        x = x.reshape(*x.shape[:2], *self.spac_dim)
        x = jnp.fft.ifftn(x, s=self.spac_dim, norm="ortho").reshape(
            *x.shape[:2], self.n_sites
        )
...
class RBMModPhase_Mom_Proj2(nn.Module):
    ...
    hidden_bias_im_init: NNInitFunc = uniform(scale=0.25 * 6.28318530718)
    site_bias_init: NNInitFunc = normal(stddev=0.01)
    out_std_init: float = 1.0
    psi_std_init: float = 0.005
    ...
    def __call__(self, inputs):
        ...
        b_re = self.param(
            "hidden_bias_re",
            self.hidden_bias_re_init,
            (self.n_sites, self.features),
            self.param_dtype,
        )
        b_im = self.param(
            "hidden_bias_im",
            self.hidden_bias_im_init,
            (self.n_sites, self.features),
            self.param_dtype,
        )
        ...
        a_re = self.param(
            "site_bias_re", self.site_bias_init, (self.n_sites, 1), self.param_dtype
        )
        a_im = self.param(
            "site_bias_im", self.site_bias_init, (self.n_sites, 1), self.param_dtype
        )
...
class RBMModPhase_Mom_Proj3(nn.Module):
    ...
    psi0_std_init: float = 0.005
    psi_mom_weight_init: float = 1.0
    psi_mom_std_init: float = 0.005
    ...
    def setup(self):
        ...
        self.kernel_init = nn.initializers.variance_scaling(
            0.5 * self.out_std_init**2, "fan_in", distribution="truncated_normal"
        )
        ...
        self.gamma_x = (
            (2.0 * self.psi0_std_init**2) / ((self.n_sites * self.features))
        ) ** (0.25)
        ...
        self.site_bias_re_init = normal(stddev=0.01)
        self.site_bias_im_init = uniform(scale=1.0 * 6.28318530718)

        gamma_w0 = math.sqrt(
            self.n_sites / (self.n_bias * (self.n_sites + 2.0 * self.beta**2))
        ) * (self.psi_mom_std_init / self.psi0_std_init)
    ...
    def __call__(self, inputs):
        ...
        b = jax.lax.complex(b_re, b_im + 0.0 * 6.28318530718)

        ap_re = self.param(
            "mom_bias_plus_re",
            self.site_bias_re_init,
            (self.n_bias, self.n_freq),
            self.param_dtype,
        )
        ap_im = self.param(
            "mom_bias_plus_im",
            self.site_bias_im_init,
            (self.n_bias, self.n_freq),
            self.param_dtype,
        )
        ...
        c_re = self.param("site0_bias_re", normal(stddev=0.01), (1,), self.param_dtype)
        c_im = self.param(
            "site0_bias_im", uniform(scale=0.01 * 6.28318530718), (1,), self.param_dtype
        )
        c = 0.5 * jnp.log(self.psi0_weight_init) + jax.lax.complex(c_re, c_im)
        ...
        wp = jnp.fft.ifftn(
            wp.reshape(-1, *self.spac_dim), s=self.spac_dim, norm="ortho"
        ).reshape(-1, self.n_sites)
        wp = wp.conj()
        wm = jnp.fft.ifftn(
            wm.reshape(-1, *self.spac_dim), s=self.spac_dim, norm="ortho"
        ).reshape(-1, self.n_sites)
        wm = wm.conj()
        ...
        y = nk.jax.logsumexp_cplx(
            jnp.stack((y0, ypp, ypm, ymp, ymm), axis=-1), None, axis=-1
        )
...
class RBMModPhase_Mom_Proj_Group_TranSF(nn.Module):
    ...
    def setup(self):
        ...
        self.gamma_x = (
            (2.0 * self.psi_std_init**2) / (self.n_group * self.features)
        ) ** (0.25)

        self.mom_bias_re_init = normal(stddev=0.01)
    ...
    def __call__(self, inputs):
        ...
        x = Point_Conv_RICO(self.features, use_bias=False, name="RBM_Point_Conv")(
            inputs
        )
        x = Depth_Conv_FFT(
            self.group_dim,
        ...
        a_re = self.param(
            "mom_bias_re",
            self.mom_bias_re_init,
            (self.features, self.n_freq),
            self.param_dtype,
        )
        a_im = self.param(
            "mom_bias_im",
            self.mom_bias_im_init,
            (self.features, self.n_freq),
            self.param_dtype,
        )
        ...
        w = jnp.fft.ifftn(
            w.reshape(self.features, *self.group_dim), s=self.group_dim, norm="ortho"
        ).reshape(self.features, self.n_group)
        w = w.T
...
class RBMModPhase_Mom_Proj_Group_TranSF2(nn.Module):
    ...
    def setup(self):
        ...
        self.mom_weight_re_init = normal(stddev=0.01)
    ...
    def __call__(self, inputs):
        ...
        a_re = self.param(
            "mom_weight_re", self.mom_weight_re_init, (self.n_freq,), self.param_dtype
        )
        a_im = self.param(
            "mom_weight_im", self.mom_weight_im_init, (self.n_freq,), self.param_dtype
        )
        ...
        w = jnp.fft.ifftn(
            w.reshape(*self.group_dim), s=self.group_dim, norm="ortho"
        ).reshape(self.n_group)

        b_re = self.param(
            "feat_bias_re", self.feat_bias_re_init, (self.features,), self.param_dtype
        )
        b_im = self.param(
            "feat_bias_im", self.feat_bias_im_init, (self.features,), self.param_dtype
        )
        b = self.gamma_b * jax.lax.complex(b_re, b_im)
...
class RBMModPhase_Mom_Proj_Group_TranSF3(nn.Module):
    ...
    def setup(self):
        ...
        self.gamma_x = ((2.0 * self.psi_std_init**2) / (self.features)) ** (0.25)
...
class RBMModPhase_Mom_Proj_Group_TranSF4(nn.Module):
    spac_dim: Tuple[int]
    features: int
    features_plus: int
    features_minus: int
    ...
    def setup(self):
        ...
        self.gamma_xp = ((2.0 * self.psi_std_init**2) / (self.features_plus)) ** (0.25)
        self.gamma_xm = ((2.0 * self.psi_std_init**2) / (self.features_minus)) ** (0.25)
        ...
        self.Dense_PR = nn.Dense(
            self.features_plus,
            param_dtype=self.param_dtype,
            kernel_init=nn.initializers.xavier_uniform(),
            bias_init=jax.nn.initializers.zeros,
        )
        self.Dense_PI = nn.Dense(
            self.features_plus,
            param_dtype=self.param_dtype,
            kernel_init=nn.initializers.xavier_uniform(),
            bias_init=jax.nn.initializers.zeros,
        )
        self.Dense_MR = nn.Dense(
            self.features_minus,
            param_dtype=self.param_dtype,
            kernel_init=nn.initializers.xavier_uniform(),
            use_bias=False,
        )
        self.Dense_MI = nn.Dense(
            self.features_minus,
            param_dtype=self.param_dtype,
            kernel_init=nn.initializers.xavier_uniform(),
            use_bias=False,
        ...
        self.scale_init = nn.initializers.constant(0.1)
        self.Norm_P0 = nn.LayerNorm(
            use_scale=True, use_bias=True, param_dtype=self.param_dtype
        )
        self.Norm_M0 = nn.LayerNorm(
            use_scale=True, use_bias=False, param_dtype=self.param_dtype
        )
        self.Norm_PR = nn.LayerNorm(
            use_scale=True,
            use_bias=True,
            scale_init=self.scale_init,
            param_dtype=self.param_dtype,
        )
        self.Norm_MR = nn.LayerNorm(
            use_scale=True,
            use_bias=False,
            scale_init=self.scale_init,
            param_dtype=self.param_dtype,
        )
        self.Norm_PI = nn.LayerNorm(
            use_scale=True,
            use_bias=True,
            scale_init=self.scale_init,
            param_dtype=self.param_dtype,
        )
        self.Norm_MI = nn.LayerNorm(
            use_scale=True,
            use_bias=False,
            scale_init=self.scale_init,
            param_dtype=self.param_dtype,
    ...
    def __call__(self, inputs):
        ...
        x = inputs.reshape(-1, *self.group_dim, features_in)
        xp = jnp.mean(x.reshape(-1, self.n_group, features_in), axis=-2)
        xm = x[:, :, :, 0, :] - x[:, :, :, 1, :]
        xm = jnp.sum(
            xm[:, 1::2, 1::2, :]
            + xm[:, ::2, ::2, :]
        ...
        xp = jax.lax.complex(
            self.Norm_PR(self.Dense_PR(xp)), self.Norm_PI(self.Dense_PI(xp))
        )
        xm = jax.lax.complex(
            self.Norm_MR(self.Dense_MR(xm)), self.Norm_MI(self.Dense_MI(xm))
        )
        ...
        xp = Point_Conv_RICO(self.features_plus,use_bias=False,name="RBM_Point_Conv_Plus")(jnp.expand_dims(xp,axis=-2) )
        xp = self.gamma_xp * xp.reshape(-1,self.features_plus)
        
        xm = Point_Conv_RICO(self.features_minus,use_bias=False,name="RBM_Point_Conv_Minus")(jnp.expand_dims(xm,axis=-2))
        xm = self.gamma_xm * xm.reshape(-1,self.features_minus)
        """
        ...
        # y=y+jnp.sum(nknn.log_cosh(xp),axis=-1)+jnp.sum(nknn.log_cosh(xm),axis=-1)
        # y=y+jnp.sum(nknn.log_cosh(xp),axis=-1)
        y = jnp.sum(nknn.log_cosh(xp), axis=-1) + jnp.sum(nknn.log_cosh(xm), axis=-1)
