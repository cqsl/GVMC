...
class RBMModPhase_Mom_Proj_Group_TranSF4(nn.Module):
    ...
    def setup(self):
      ...
      self.gamma_x = ( ( 2. * self.psi_std_init**2 ) / (    self.n_group * self.features       ) )**(.25)
      self.gamma_xp = ( ( 2. * self.psi_std_init**2 ) / (     self.features_plus      ) )**(.25)
      self.gamma_xm = ( ( 2. * self.psi_std_init**2 ) / (    self.features_minus       ) )**(.25)
      ...
      self.scale_init =nn.initializers.constant(.1)
      self.Norm_P0 = nn.LayerNorm(use_scale=True, use_bias=True, param_dtype=self.param_dtype)
      self.Norm_M0 = nn.LayerNorm(use_scale=True, use_bias=False, param_dtype=self.param_dtype)
      self.Norm_PR = nn.LayerNorm(use_scale=True, use_bias=True,scale_init=self.scale_init, param_dtype=self.param_dtype)
      self.Norm_MR = nn.LayerNorm(use_scale=True, use_bias=False,scale_init=self.scale_init, param_dtype=self.param_dtype)
      self.Norm_PI = nn.LayerNorm(use_scale=True, use_bias=True,scale_init=self.scale_init, param_dtype=self.param_dtype)
      self.Norm_MI = nn.LayerNorm(use_scale=True, use_bias=False,scale_init=self.scale_init, param_dtype=self.param_dtype)
    ...
    def __call__(self, inputs):
        ...
        xm =  x[:,:,:,0,:] - x[:,:,:,1,:] 
        xm = jnp.sum(  xm[:,1::2, 1::2,:] + xm[:,::2, ::2,:]- xm[:,1::2, ::2,:] -xm[:,::2, 1::2,:] ,axis=(-2,-3)) 
        xm = xm /self.n_group

        xp=self.Norm_P0(xp)
        xm=self.Norm_M0(xm)

        xp = jax.lax.complex( self.Norm_PR(self.Dense_PR(xp)) , self.Norm_PI(self.Dense_PI(xp)) )
        xm = jax.lax.complex( self.Norm_MR(self.Dense_MR(xm)) , self.Norm_MI(self.Dense_MI(xm)) )
