...
def create_model(meta_values):
    ...
    spac_dim, group_dim, k_vec = (
        meta_values["spac_dim"],
        meta_values["group_dim"],
        meta_values["k_vec"],
    )

    stage_sizes, features_in, stage_features = (
        meta_values["stage_sizes"],
        meta_values["features_in"],
        meta_values["stage_features"],
    )
    kernel_width, kernel_width_in = (
        meta_values["kernel_width"],
        meta_values["kernel_width_in"],
    )
    sign_symm, sign_symm_pool = meta_values["sign_symm"], meta_values["sign_symm_pool"]

    RBM_features_out, RBM_use_bias = (
        meta_values["RBM_features_out"],
        meta_values["RBM_use_bias"],
    )
    ...
    Out_Module = partial(
        RBMModPhase_Mom_Proj_Group_TranSF2,
        spac_dim,
        RBM_features_out,
        use_hidden_bias=RBM_use_bias,
        psi_std_init=0.02,
        mask=freq_mask,
    )
    model_vec = Grass_VWF_Vec_BF(BF_Module, Out_Module, nvwf, spac_dim)
