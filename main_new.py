...
#ham = Ham_Heisenberg_J1_J2_pcl(hilb,spac_dim,J2=.25*.50,msr=True)
...
lr0=0.15 #learning rate /step size (Ham is normalized by 1/n_sites so not that big )
lam0=1e-03 # QGT matrix shift /regularization (forgot why it is lam0 not just lam but there is no lam anymore)
#lam0= 0.0
mu=.9 #SPRING decay/momentum term (settting to 0 gives plain minSR)( can be increased to upto about .99 may help or make worse)
nch=2**10 #n chanins
chl=2**1 #chain length / samps per chain
nsamp=nch*chl # total samps
iterations=8000 #weight updates
#iterations=0
...
symm_prop = SymmetryProposal_2D_Square_SpinHalf(spac_dim,.1)
...
for it in range(iterations):
	...
	if it%50 == 0:
		print(f'it: {it} (E_avg,E_std): ({E_avg:.6f},{E_std:.6f}) (E_ravg,E_rstd): ({E_ravg:.8f},{E_rstd:.6f}) E_eig: {E_eig}:  mdp: {avg_dparam} acc: {100.0  *sampler_state.acceptance: .1f}%   pln: ({pln_min},{pln_max}) J_norm:{J_norm} time:{it_time}',flush=True)
