...
def Ham_Heisenberg_pcl(hilb, latt_dim, J_list, brill_vecs, msr=False):
    ...
    for J, bvec in zip(J_list, brill_vecs):
        ...
        for i in range(np.prod(latt_dim)):
            ...
            ham = ham + J * (
                nk.operator.spin.sigmaz(hilb, i) @ nk.operator.spin.sigmaz(hilb, j)
            )
            ham = ham + 2.0 * xys * J * (
                nk.operator.spin.sigmap(hilb, i) @ nk.operator.spin.sigmam(hilb, j)
            )
            ham = ham + 2.0 * xys * J * (
                nk.operator.spin.sigmam(hilb, i) @ nk.operator.spin.sigmap(hilb, j)
            )

    return ham.to_jax_operator()


def Ham_Heisenberg_J1_pcl(hilb, latt_dim, J1=0.25, msr=False, norm_per_site=True):
...
def Ham_Heisenberg_J1_J2_pcl(
    hilb, latt_dim, J1=0.25, J2=0.0, msr=False, norm_per_site=True
):
...
def Ham_TF_Ising_pcl(hilb, latt_dim, J=1.0, g=0.5, norm_per_site=True):
    ...
    ham = nk.operator.LocalOperator(hilb)
    for i in range(np.prod(latt_dim)):
        ham = ham - (0.5 * g) * nk.operator.spin.sigmax(hilb, i)
        vi = np.array(np.unravel_index(i, latt_dim))
        for J, bvec in zip(J_list, brill_vecs):
            vj = (vi + bvec + latt_dim) % latt_dim
            j = np.ravel_multi_index(vj, latt_dim)
