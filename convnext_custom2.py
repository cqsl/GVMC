...
from depth_conv_custom import Depth_Conv_FFT, Depth_Conv_FFT_Real
...
class Depth_Conv(nn.Module):
    ...
    def __call__(self, inputs):
        return nn.Conv(
            ...
            kernel_init=default_dconv_init,
            name="dConv",
        )(inputs)
...
class GRN(nn.Module):
    ...
    def __call__(self, x):
        ...
        beta = self.param("beta", nn.initializers.zeros, (features,), self.param_dtype)
        """
        ...
        Gx = jnp.linalg.norm(
            x.reshape(-1, n_sites, features), axis=1, keepdims=True
        ).reshape(-1, *gamma_shape, features)
...
class ConvNeXtBlock(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        x = nn.Dense(
            self.feature_multiplier * self.features,
            kernel_init=default_pwconv_init,
            param_dtype=defualt_dtype,
            name="pwconv0",
        )(x)
        x = self.act(x)
        x = GRN()(x)
...
class ConvNeXt2(nn.Module):
    ...
    sign_symm: bool = (
        False  # whether to enforse sign symmetry. If true for all inputs x,  f(-x) =-f(x)
    )
    ...
    def __call__(self, inputs):
        ...
        for i, (block_size, block_features) in enumerate(
            zip(self.stage_sizes, self.stage_features)
        ):
    ...
    """
        if self.sign_symm and self.sign_symm_pool:
            x = jax.lax.index_in_dim(
                x, 0, axis=-2, keepdims=False
            ) - jax.lax.index_in_dim(x, 1, axis=-2, keepdims=False)
        ...
        if self.global_pool:
            ...
            x = norm(name="bn_final", use_scale=False, use_bias=False)(x)
            # x = norm(name="bn_final",use_scale=True, use_bias=False )(x)
