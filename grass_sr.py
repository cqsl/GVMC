...
def update_params(J, B, params, opt_vars, params_unravel, nvwf):
    ...
    lr, it, lam0, mu, params_arr_grad0 = (
        opt_vars["lr"],
        opt_vars["it"],
        opt_vars["lam0"],
        opt_vars["mu"],
        opt_vars["params_arr_grad0"],
    )
    # lr_it = lr*( 1 / (100+it) )
    lr_it, lam0_it, mu_it = lr, lam0, mu
    G_diag = opt_vars["diag_norm"]

    params_arr_grad = minSR_solve_Kacz(
        J, B, params_arr_grad0, lam0=lam0_it, lam1=1.0, mu=mu_it
    )
  ...
  """
    ...
    params = jax.tree_util.tree_map(
        lambda pars, grad: pars - lr_it * grad, params, params_grad
    )
    opt_vars = {
        "params_arr_grad0": params_arr_grad0,
        "diag_norm": G_diag,
        "lr": lr,
        "lam0": lam0,
        "mu": mu,
...
def vwf_vec_jacobian(
    ...
    *,
    ...
    def jac_fun_single(fun, params, sample, v):
        ...
        return ravel_no_uravel(
            jax.tree_util.tree_map(jax.lax.complex, dp_re_pyt, dp_im_pyt)
        )

    # J = jax.vmap(jac_fun_single , in_axes=(None,None,0,0) ,out_axes=0)( f, vec_params,samples,VJ)
    J = vmap_chunked(jac_fun_single, in_axes=(None, None, 0, 0), chunk_size=chunk_size)(
        f, vec_params, samples, VJ
    )
...
def pinv_smooth(
    A, b, *, rcond: float = 1e-14, rcond_smooth: float = 1e-6, rcond_power: int = 6
):
...
# @jax.jit
def minSR_solve(J, b, lam0=1e-04, lam1=1.0):
...
# @jax.jit
def minSR_solve_Kacz(J, b, x0, lam0=1e-04, lam1=1.0, mu=0.99):
