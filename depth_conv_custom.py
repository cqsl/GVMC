...
class Depth_Conv_FFT(nn.Module):
    ...
    kernel_init: NNInitFunc = nn.initializers.variance_scaling(
        is2,
        "fan_in",
    ...
    def __call__(self, inputs):
        ...
        if self.use_bias:
            if self.real_in_out:
                ...
                bias_re = self.param(
                    "bias_re", self.bias_init, (self.features,), self.param_dtype
                )
                bias_im = self.param(
                    "bias_im", self.bias_init, (self.features,), self.param_dtype
                )
