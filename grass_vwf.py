...
class Grass_VWF_Vec_BF(Module):
    BF_Module: Type[
        Module
    ]  # Shared backflow modulde whose weights are shared by all vwfs
    Ind_Module: Type[
        Module
    ]  # Invdivdual module which is vectorized / duplicated for each vwf with a seperate set of weights.
    n_model: int  # number of vwfs (could be changed to n_vwf)
    spac_dim: Tuple[
        int
    ]  # tuple of spatial/lattice dimensions of the input states: prod(spac_dim) = n_sites
    features_in: int = (
        1  # optional additional feature dim for inputs (in most cases will be 1 can also be used for bravais lattices index )
    )
    ...
    def __call__(self, inputs):
        ...
        x = inputs.reshape(
            *batch_dim, *self.spac_dim, self.features_in
        )  # Input basis states are assumed to be of the form of Netket sampler
        ...
        x = back_flow(name="bf_params")(
            x
        )  # inputs are fed into shared back_flow modulde with dict key "bf_params"
        ...
        return (
            x - self.variables["norm"]["log_norm"]
        )  # output is normalized by log_norm ie. log(psi_i(x)) -> log(psi_i(x)) - log_norm_i
...
def overlap_mat(mat_log_elem):
    ...
    row_norm_overlap_mat = jnp.exp(
        mat_log_elem - jnp.expand_dims(row_log_norm, axis=-1)
    )
...
def operator_overlap_mat(
    mod_apply_fn, params, state, sigma, row_log_norm, Jax_Op, chunk_size=1
):
    ...
    def oomb(sigma_batch, rln):
        ...
        Op_Phi_batch = jnp.sum(
            jnp.expand_dims(Op_mat_elem, axis=-1) * jnp.exp(log_Phi_eta - rln), axis=-2
        )
        return Op_Phi_batch

    # Netket's chunked vmap of "oomb"
    Op_Phi = vmap_chunked(oomb, in_axes=(0, 0), chunk_size=chunk_size)(
        sigma, jnp.expand_dims(row_log_norm, axis=-1)
    )
    return Op_Phi
...
def renorm_Phi(log_Phi_sigma, vec_state):
    ...
    dpln = jnp.mean(jnp.exp(2.0 * (jnp.real(log_Phi_sigma) - a)), axis=(0, 1))
    dpln = a + 0.5 * jnp.log(dpln)
    # dpln *= .00
...
def det_and_inv_row_update(A0_inv, A_row_k, k):
    ...
    v = (1.0 / det_ratio) * (v.at[k].add(-1.0))
