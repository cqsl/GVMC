...
class SymmetryProposal_2D_Square_SpinHalf(SymmetryProposal):
    ...
    def transition(self, key, inputs):
        ...
        group_ind = jax.random.randint(
            key_group, shape=(n_samp,), minval=0, maxval=self.n_group
        )
        g_ind_vec = jnp.stack(jnp.unravel_index(group_ind, self.group_dim), axis=-1)

        spins = jax.vmap(jnp.roll)(
            inputs.reshape(n_samp, *self.spac_dim), g_ind_vec[:, 0:2]
        )
        spins = jax.vmap(
            lambda s, nr: jax.lax.cond(
                nr == 1, lambda x: jnp.rot90(x, k=1), lambda x: x, s
            )
        )(spins, g_ind_vec[:, 2])
        spins = jax.vmap(
            lambda s, nr: jax.lax.cond(
                nr == 2, lambda x: jnp.rot90(x, k=2), lambda x: x, s
            )
        )(spins, g_ind_vec[:, 2])
        spins = jax.vmap(
            lambda s, nr: jax.lax.cond(
                nr == 3, lambda x: jnp.rot90(x, k=3), lambda x: x, s
            )
        )(spins, g_ind_vec[:, 2])
        spins = jax.vmap(lambda s, nr: jax.lax.cond(nr, jnp.flipud, lambda x: x, s))(
            spins, g_ind_vec[:, 3]
        )
        spins = jax.vmap(lambda s, nf: jax.lax.cond(nf, lambda x: -x, lambda x: x, s))(
            spins, g_ind_vec[:, 4]
        )
...
class WedgeRule(MetropolisRule):
    ...
    hilbert_ind: AbstractHilbert = struct.field(
        pytree_node=False
    )  # invidual/single hilbert space
    hilbert: TensorHilbert = struct.field(
        pytree_node=False
    )  # hilbert = hilbert_ind**nvwf hilberst space for this class
    rule: MetropolisRule  # chosen rule (Local,Exchange,etc.) defined over  hilbert_ind
    # symm_prop: Union[Callable , None] = None
    symm_prop: Union[SymmetryProposal, None] = (
        None  # a mess of class that applies random symmetry transformations on top of rule
    )

    def __init__(
        self,
        hilbert_ind: AbstractHilbert,
        n_wedge: int,
        rule: MetropolisRule,
        symm_prop: Union[Callable, None] = None,
    ) -> "TensorRule":
        ...
        self.hilbert = TensorHilbert(
            *(hilbert_ind,) * n_wedge
        )  # n_wedge = n_vwf as the number of hilbert copies
        self.rule = rule
    ...
    def init_state(
        ...
        return self.rule.init_state(
            sampler.replace(hilbert=self.hilbert_ind), machine, params, key
        )
    ...
    def reset(
        ...
        return self.rule.reset(
            sampler.replace(hilbert=self.hilbert_ind), machine, params, sampler_state
        )
    ...
    def transition(self, sampler, machine, parameters, state, key, σ):
        ...
        I = jax.random.randint(key_I, shape=(σ.shape[0],), minval=0, maxval=N)
        ...
        σp_I = jax.vmap(lambda sig, i: jax.lax.dynamic_slice_in_dim(sig, L * i, L))(
            σ, I
        )
        σp_I, log_prob_corr = self.rule.transition(
            sampler.replace(hilbert=self.hilbert_ind),
            machine,
            parameters,
            state,
            key_tran,
