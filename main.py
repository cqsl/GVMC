...
from grass_samp import MetropolisSampler_Wedge, MetropolisExchange_Wedge
from grass_samp_rule import SymmetryProposal_2D_Square_SpinHalf
...
# forget about create_model2 just use create_model which what all the saved models in the paper used
...
n_sites = L**n_dim
spac_dim = (L,) * n_dim
nvwf = 2  # number indivdual vwfs
print(f" {n_dim}D sq. latt. length {L} for {nvwf} ex. states")

# I only wrote code for translational symmetry + spin flip
group_dim = spac_dim + (2,)
n_group = np.prod(group_dim)

# this only needed for passing to vwf model to give the right symmetry phases
k_vec = (
    0,
    0,
    0,
)  # integer momentum vec momentum is 2pi/L*(k1,k2) and k3=0 or 1 for +/- spin flip phases
pi2 = 6.28318530718
phases = (
    np.exp(0.0 + 1j * 0.0),
    np.exp(1j * pi2 * (k_vec[0] / L)),
    np.exp(1j * pi2 * (k_vec[1] / L)),
    np.exp(1j * pi2 * (k_vec[2] / 2)),
)
...
file_name_save = (
    file_name_save + "_k_" + "_".join([f"{n}" for n in k_vec]) + f"_nvwf_{nvwf}"
)
...
# lam0= 0.0
mu = 0.9  # SPRING decay/momentum term (settting to 0 gives minSR)( can be increased to upto about .99 may help or make worse)

# small samples for testing
nch = 2**6  # num of chanins
chl = 2**0  # chain length / samps per chain
# samples I did for runs in paper
# ch=2**10
# chl=2**2
nsamp = nch * chl  # total samps

n_warmup = 1  # number MCMC warmup iterations before starting param updates
# If you want a totally new Markov chains for a loaded model increase by alot
# ie load_params=True and load_sampler=False
...
chunk_size_energy = (
    2**8
)  # chunk size for calculating local energy matrices (one nvwfxnvwf matrix =1 chunk)
chunk_size_derv = (
    2**10
)  # chunk size for calculating jacobian (cant remember how one chunk is defined but not as much of a mem. issue)
...
# saved model used in paper for 6x6
# 6x6 model_0 beta=.9,lr=.15,lam=10-3,psi_std=.02
# this model has 2+2+8 = 12 resnet blocks w/ features sizes of 4,6,12 respectives
# The indvidual out RBM features are 16 ie alpha = 16/12=1.333
"""
...
"""
# saved model used in paper for 10x10
...
# very low param model for testing
...
def count_params(params):
    n_leaves = jax.tree_util.tree_leaves(
...
# individual vwf coeffeicnts correspond to last dim of output array
...
# prints model pytree of array dims
# print(pyt_shape(vec_vars['params']))
nparam = count_params(vec_params)  # total num of params
nparam_bf = count_params(vec_params["bf_params"])  # num of backflow params
nparam_ind = count_params(
    vec_params["ind_params"]
)  # total num of params for indvidual heads
...
# dicts that are passed to and updated by excited_update_step
# contains the learning rate, SPRING momentum,... and the previous iteration grad
opt_vars = {
    "params_arr_grad0": np.zeros((nparam,), dtype=param_dtype),
    "diag_norm": np.zeros((nparam,)),
    "lr": lr0,
    "lam0": lam0,
    "mu": mu,
    "it": 0,
}
# basic values energy,variance... to monitor runs
grass_vars = {
    "E_avg": 0.0 + 0.0j,
    "E_var": 0.0,
    "H": np.zeros((nvwf, nvwf), dtype=np.complex128),
    "H_var": np.zeros((nvwf, nvwf), dtype=np.complex128),
}


symm_prop_prob = (
    0.1  # prob of proposed state being a symmetry transformation of the original state
)
...
# creates a custom netket sampler for the deteminant sampling
...
sampler_state = sampler.init_state(
    mod_apply_fn_vec, {"params": vec_params, **vec_state}
)
sampler_state = sampler.reset(
    mod_apply_fn_vec, {"params": vec_params, **vec_state}, sampler_state
)
...
if load_params:
    ...
    vec_params, vec_state, opt_vars, grass_vars = (
        save_values["params"],
        save_values["state"],
        save_values["opt"],
        save_values["grass"],
...
for it in range(iterations):
    ...
    vec_params, vec_state, opt_vars, grass_vars = excited_update_step(
        mod_apply_fn_vec,
        params_unravel,
        sigma,
        ham,
    ...
    if it % 50 == 0:
        print(
            f"it: {it} (E_avg,E_std): ({E_avg:.6f},{E_std:.6f}) (E_ravg,E_rstd): ({E_ravg:.8f},{E_rstd:.6f})  acc: {100.0  *sampler_state.acceptance: .1f}% time:{it_time}",
            flush=True,
        )
        print(E_eig)
