...
meta_values={ 'spac_dim': spac_dim , 'group_dim': group_dim , 'k_vec': k_vec, 'nvwf': nvwf,
             'stage_sizes': [4],'features_in':4,'stage_features':[6],
             'kernel_width_in': 3,'kernel_width': 3,
             'sign_symm': True, 'sign_symm_pool':False,
             'RBM_features_out': 5, 'RBM_use_bias': True,
            }
...
nparam_ind = count_params(vec_params['ind_params']) #total num of params for indvidual heads 
...
for it in range(iterations):
	...
	if it%50 == 0:
		print(f'it: {it} (E_avg,E_std): ({E_avg:.6f},{E_std:.6f}) (E_ravg,E_rstd): ({E_ravg:.8f},{E_rstd:.6f})  acc: {100.0  *sampler_state.acceptance: .1f}% time:{it_time}',flush=True)
		print( E_eig)
