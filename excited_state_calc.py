...
@partial(
    jax.jit,
    static_argnames=["mod_apply_fn_vec", "params_unravel", "chunk_sizes", "mode"],
)
def excited_update_step(
    mod_apply_fn_vec,
    params_unravel,
    sigma,
    ham,
    ...
    HPhi = gvwf.operator_overlap_mat(
        mod_apply_fn_vec,
        vec_params,
        vec_state,
        sigma,
    ...
    J = gsr.vwf_vec_jacobian(
        mod_apply_fn_vec,
        vec_params,
        sigma,
        Phi * Phi_inv,
        vec_state,
        chunk_size=chunk_size_derv,
    )
  ...
  """
    ...
    vec_params, opt_vars = gsr.update_params(
        J, B, vec_params, opt_vars, params_unravel, nvwf
    )
